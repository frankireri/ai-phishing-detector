      ...
      <div className="w-full max-w-5xl">
        <header className="mb-12 flex flex-col items-center text-center relative">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tighter text-black leading-tight">
            Scanner Workspace<span className="text-orange-500">.</span>
          </h1>
        </header>
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-100/50">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {result.suspicious_words.map((word, idx) => (
                          <span 
                            key={idx} 
                            className="group relative bg-orange-100 text-orange-700 text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-sm border border-orange-200 cursor-help"
                          >
                            {word}
                            <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-black text-white text-center text-[10px] py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                              This word appeared highly suspicious in the analysis vector.
                            </span>
                          </span>
                        ))}
                      </div>
              ...
              {showHowItWorks && (
                <div className="bg-white border-x-2 border-b-2 border-black p-8 rounded-b-sm shadow-sm text-sm text-gray-600 leading-relaxed mt-[-2px]">
                  <ol className="list-decimal list-inside space-y-4">
                    ...
                    <li><strong className="text-black">TF-IDF Vectorization:</strong> The cleaned text is converted into a numerical vector using Term Frequency-Inverse Document Frequency.</li>
                    <li><strong className="text-black">Random Forest Classifier:</strong> A powerful ensemble machine learning model analyzes the feature vector across 100 decision trees.</li>
                    <li><strong className="text-black">Continuous Learning:</strong> By reporting incorrect results, the message is automatically queued into the database to retrain the Random Forest model.</li>
                  </ol>
                </div>
              )}
            </div>
